...
std::vector<ghc::filesystem::path> split_path(const std::string_view& path_env) {
  ...
  while (segment_begin != path_env.size()) {
    ...
    if (segment_end == std::string_view::npos) {
      search_path.push_back(path_env.substr(segment_begin, path_env.size() - segment_begin));
      break;
    }
    else {
      search_path.push_back(path_env.substr(segment_begin, segment_end - segment_begin));
...
std::optional<ghc::filesystem::path> search_in_path(const std::vector<ghc::filesystem::path>& path, const std::string_view& target) {
...
std::optional<std::string_view> ProcessEnvironment::get(const std::string_view& key) const {
  for (const auto& variable : variables_) {
    if (variable.starts_with(key) && variable.size() > key.size() &&
      variable[key.size()] == '=') {
...
Process::Handle::Handle(pid_t pid) :
  pid_(pid) {
}
